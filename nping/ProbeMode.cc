       ...
       case NSE_TYPE_TIMER:
            if( mypacket!=NULL){
                ...
                if( o.showSentPackets() ){
                    nping_print(VB_0,"SENT (%.6fs) %s", o.stats.elapsedRuntime(t), pktinfobuffer );
       ...
       case NSE_TYPE_PCAP_READ:
            ...
            if( nsock_iod_linktype(nsi) == DLT_EN10MB ){
                ...
                *ethtype=ntohs(*ethtype);
                switch(*ethtype){
                    ...
                    break;
                    default:
                        nping_warning(QT_1, "RCVD (%.6fs) Unsupported protocol (Ethernet type %02X)", o.stats.elapsedRuntime(t), *ethtype);
             ...
             * IP packets (the BPF filter prevents that) */
                ...
                if( iphdr.storeRecvData(packet, packetlen)!=OP_SUCCESS )
                    nping_warning(QT_1, "RCVD (%.6fs) Bogus packet received.", o.stats.elapsedRuntime(t));
                if( iphdr.getVersion()==4 || iphdr.getVersion()==6){
                    ip=true;
                }else{
                    nping_warning(QT_1, "RCVD (%.6fs) Unsupported protocol.", o.stats.elapsedRuntime(t));
                    print_hexdump(VB_3, packet, packetlen);
                    return;
                }
            }
            ...
            if(ip){
                ...
                if (proto == IPPROTO_UDP || proto == IPPROTO_TCP){
                    ...
                    opened */
                    snprintf(final_output, sizeof(final_output), "RCVD (%.6fs) %s\n", o.stats.elapsedRuntime(t), buffer);
                    if( o.getVerbosity() >= VB_3 ){
            ...
            /* Packet is ARP */
                ...
                nping_print(VB_0, "RCVD (%.6fs) %s", o.stats.elapsedRuntime(t), buffer );
                o.stats.addRecvPacket(packetlen);
  ...
  switch(type) {
    ...
    case NSE_TYPE_CONNECT:
        ...
        if(trg!=NULL){
            if ( trg->getSuppliedHostName() )
                nping_print(VB_0,"RCVD (%.6fs) Handshake with %s:%d (%s:%d) completed",
    ...
    case NSE_TYPE_TIMER:
        ...
        if( o.showSentPackets() ){
            if ( mypacket->target->getSuppliedHostName() )
                nping_print(VB_0,"SENT (%.6fs) Starting TCP Handshake > %s:%d (%s:%d)", o.stats.elapsedRuntime(NULL), mypacket->target->getSuppliedHostName(), mypacket->dstport ,mypacket->target->getTargetIPstr(), mypacket->dstport);
            else
                nping_print(VB_0,"SENT (%.6fs) Starting TCP Handshake > %s:%d", o.stats.elapsedRuntime(NULL), mypacket->target->getTargetIPstr(), mypacket->dstport);
        }
        packetno++;
    ...
    * why we say "Possible TCP RST received". */
    if ( type == NSE_TYPE_CONNECT ){
        ...
        nping_print(VB_0,"RCVD (%.6fs) Possible TCP RST received from %s:%d --> %s", o.stats.elapsedRuntime(t),ipstring, peerport, strerror(nse_errorcode(nse)) );
     }
     else
 ...
 if (status == NSE_STATUS_SUCCESS ) {
  ...
  switch(type) {
    ...
    case NSE_TYPE_WRITE:
        ...
        if(trg!=NULL){
            if ( trg->getSuppliedHostName() )
                nping_print(VB_0,"SENT (%.6fs) UDP packet with %lu bytes to %s:%d (%s:%d)", o.stats.elapsedRuntime(NULL),  (unsigned long int)sentbytes, trg->getSuppliedHostName(), peerport, ipstring, peerport );
            else
                nping_print(VB_0,"SENT (%.6fs) UDP packet with %lu bytes to %s:%d", o.stats.elapsedRuntime(NULL),  (unsigned long int)sentbytes, ipstring, peerport );
            trg->setProbeSentUDP( 0, peerport);
        }else{
            nping_print(VB_0,"SENT (%.6fs) UDP packet with %lu bytes to %s:%d", o.stats.elapsedRuntime(t), (unsigned long int)sentbytes, ipstring, peerport );
        }
    ...
    case NSE_TYPE_READ:
        ...
        if(trg!=NULL){
            if ( trg->getSuppliedHostName() )
                nping_print(VB_0,"RCVD (%.6fs) UDP packet with %d bytes from %s:%d (%s:%d)", o.stats.elapsedRuntime(NULL),  readbytes, trg->getSuppliedHostName(), peerport, ipstring, peerport );
            else
                nping_print(VB_0,"RCVD (%.6fs) UDP packet with %d bytes from %s:%d", o.stats.elapsedRuntime(NULL),  readbytes, ipstring, peerport );
            trg->setProbeRecvUDP(peerport, 0);
        }else{
            nping_print(VB_0,"RCVD (%.6fs) UDP packet with %d bytes from %s:%d", o.stats.elapsedRuntime(t), readbytes, ipstring, peerport );
        }
